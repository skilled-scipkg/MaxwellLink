...
def main(args):
    ...
    def pw_amp(k, x0):
        def _pw_amp(x):
            # we increase the amplitude by 5e4 to ensure sufficient excitation of molecules (matching the 2D-IR amplitude)
            return 5e4 * cmath.exp(1j * 2 * math.pi * k.dot(x + x0))
        return _pw_amp
    ...
    # for productive run with molecules, we turn off flux calculations and savings, and reduce the tmax to 120
    #nfreq = 200
