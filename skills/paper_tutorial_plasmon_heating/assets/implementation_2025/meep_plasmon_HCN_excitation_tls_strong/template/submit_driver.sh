...
if [ "$nmol" -gt 1 ]; then
    ...
    for ((i=1; i<nmol; i++)); do
        ...
        mxl_driver --model tls --address $HOST --port $PORT --param "omega=0.0157904, mu12=0.15, orientation=1, pe_initial=0.0" &
    done
    ...
    mxl_driver --model tls --address $HOST --port $PORT --param "omega=0.0157904, mu12=0.15, orientation=1, pe_initial=0.0" --verbose
fi
